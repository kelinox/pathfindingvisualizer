class AStar {
  ...
  execute(nodes, start, end) {
    this.start = start;
    this.end = end;
    this.graph = this.createGraph(nodes, end);
        ...
        if (start[0] !== neighbors[i][0] || start[1] !== neighbors[i][1]) {
          ...
          if (!n.obstacle && n.distanceToStart === Number.MAX_SAFE_INTEGER)
            this.graph[neighbors[i][0] * 50 + neighbors[i][1]].distanceToStart =
              currentNode.distanceToStart + 1 + currentNode.distanceToEnd;
        }
      }
  ...
  /**
   * Create the graph as an int[]
   * @param {int[][]} nodes Array with the row and columns composing the graph
   * @param {int[]} end Coordinates of the ending node (row, column)
   */
  createGraph(nodes, end) {
    var graph = [];
    for (let row = 0; row < 20; row++) {
      for (let col = 0; col < 50; col++) {
        graph.push({
          distanceToStart: nodes[row][col].distanceToStart,
          distanceToEnd: Math.abs(end[0] - row) + Math.abs(end[1] - col),
          visited: false,
